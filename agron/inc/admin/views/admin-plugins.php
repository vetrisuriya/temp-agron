...
$status_ins = false;
$status_act = false;
$btn_text = esc_html__('Install All', 'agron');
...
$merlin_setup = get_option( 'merlin_' . agron()->get_slug() . '_completed' );
 
if( $status_ins && $status_act)
	$btn_text = esc_html__('Install & Active All', 'agron');
else if($status_ins && !$status_act)
	$btn_text = esc_html__('Install All', 'agron');
else if(!$status_ins && $status_act)
	$btn_text = esc_html__('Active All', 'agron');
?>
<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		<header class="pxl-dsb-header admin-plugin">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Install Plugins', 'agron' ); ?></h4>
			...
			<p><?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'agron' ); ?></p> 
		</header>
	        ...
	        <?php
				...
				foreach( $plugins as $plugin ) :
					...
					elseif( !is_plugin_inactive( $file_path ) ) {
						...
						$display_status = esc_html__( 'Active:', 'agron' );
					}
			?>
				<div class="pxl-col pxl-col-3">
					...
					<?php 
					$barplugin = new Agron_Admin_Plugins;
