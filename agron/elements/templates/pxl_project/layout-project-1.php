<?php
    $post_type = $widget->get_setting('post_type', ['project']);
    $tax = ['project-category'];
    $layout = $widget->get_setting('layout_'.$post_type, 'project-1');
    ...
    if($img_dimension === 'custom') {
        ...
        $img_dimension = (!empty($custom_img_dimension['width']) && !empty($custom_img_dimension['height'])) ? $custom_img_dimension : ['width' => 767, 'height' => 906];
    }
    ...
    $num_of_words = $widget->get_setting('num_of_words', 18);
    $show_button = (bool)$widget->get_setting('show_button', '');

    $layout_type = $widget->get_setting('layout_type', 'grid');
    $layout_style = 'layout-'.$widget->get_setting('layout_style', 'default');
    ...
    if($layout_type === 'grid') : 
        $grid_items = $widget->get_setting('grid_items', []);
        ...
        $wrap_attrs = [
            'class'            => 'grid pxl-project pxl-project-grid pxl-project-layout1 '.$layout_style,
...
?>
<div class="pxl-swiper pxl-project pxl-project-carousel pxl-project-layout1 <?php echo esc_attr($layout_style); ?>">
